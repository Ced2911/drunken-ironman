...
FILE2STR = btools/file2str$(BUILD_EXE)
MAKEDEP = btools/makedep$(BUILD_EXE)
MAKELIST = btools/makelist$(BUILD_EXE)
MAKEDEV = btools/makedev$(BUILD_EXE)
PNG2BDC = btools/png2bdc$(BUILD_EXE)
VERINFO = btools/verinfo$(BUILD_EXE)

-include $(SRC)/osd/$(CROSS_BUILD_OSD)/build.mak
#include $(SRC)/tools/tools.mak

# combine the various definitions to one
CCOMFLAGS += $(INCPATH) $(XENON_INCPATH)
CDEFS = $(DEFS)
...
run:$(EMULATOR)
	cp $(EMULATOR)32 /srv/tftp/tftpboot/xenon
	xenon-strip /srv/tftp/tftpboot/xenon
#-------------------------------------------------
# directory targets
#-------------------------------------------------
...
#	$(LD) $(LDFLAGS) $(LDFLAGSEMULATOR) $^ $(LIBS) -o $@
	$(LD) -g $(MACHDEP) -Wl,-Map,$(notdir $@).map $(LDFLAGS) $(LDFLAGSEMULATOR) $(LIBPATHS)  $(LIBS) -lxenon -lm  -lpng -lfreetype $^  -lxenon -lpng -lfreetype -lbz2 -lfat  -n -T $(LDSCRIPT) -o $@
